...
def map3_interval(q, rp):
    """Forward interval alignment + reverse/conjugate read; either may witness."""
    if q is None or rp is None or len(q['iv']) == 0 or len(rp['iv']) == 0:
        return 0.0, 0.0
    if q['niw'] == 0 or rp['niw'] == 0:
        return 0.0, 0.0
    norm = q['niw'] * rp['niw']
    fwd = _align(q['iv'], q['iw'], rp['iv'], rp['iw']) / norm
    rev = _align(q['iv'], q['iw'], rp['iv'][::-1], rp['iw'][::-1]) / norm
...
def _self_test():
    ...
    s3, gap = map3_interval(a, b)
    print(f"self-test identical spectra: map2={s2:.4f} map3={s3:.4f} gap={gap:.4f}", flush=True)
    assert s2 > 0.99, f"map2 self-test failed: {s2}"
    assert s3 > 0.99, f"map3 self-test failed: {s3}"
    assert gap > 0.3, f"map3 conjugate-gap test failed: {gap}"
...
print("=== Rung 2 corrected benchmark (v2): no Cartesian measure of phi maps ===", flush=True)
...
for qi, (true_smi, qs) in enumerate(queries):
    ...
    sc, gap_of = score_all(qp, cands)
    t = {k: top1(sc[k]) for k in sc}
    if t['map3'] in gap_of:
        conj_gaps.append(gap_of[t['map3']])
    for k in map_hits:
        if t[k] == true_smi:
            map_hits[k] += 1
...
L = []
L.append("RUNG 2 CORRECTED BENCHMARK (v2) — 2026-09-17")
...
L.append("")
conj_gaps = np.array(conj_gaps)
L.append(f"CONJUGATE GAP (fwd-rev at map3 top call): mean {conj_gaps.mean():.3f}, "
         f"median {np.median(conj_gaps):.3f}")
L.append("")
...
with open(os.path.join(ROOT, 'models', 'phi_frame_v2_report.txt'), 'w') as f:
    f.write(report + "\n")
print("report -> models/phi_frame_v2_report.txt", flush=True)
